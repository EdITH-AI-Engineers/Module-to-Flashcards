...
def test_relation_labels_are_limited_to_controlled_vocabulary():
    ...
    assert text_extractor.normalize_relation("inception") is None


def test_source_grounding_rejects_absent_entities_and_reversed_order():
    assert text_extractor._source_supports_order(
        "RA 8044",
        "created_by",
        "NYC",
        "RA 8044 created NYC.",
    )
    assert not text_extractor._source_supports_order(
        "nationalist European Union",
        "part_of",
        "Brexit",
        "Brexit changed political debate.",
    )
    assert not text_extractor._source_supports_order(
        "NYC",
        "created_by",
...
def test_entity_aliases_share_one_canonical_node(tmp_path):
    triples = [
        {
            "subject": "RA 8044",
            "relation": "defines",
            "object": "Youth in Nation-Building Act",
            "evidence": [],
        },
        {
            "subject": "Republic Act 8044",
            "relation": "enacted_on",
            "object": "1995",
            "evidence": [],
        },
    ...
    assert sum(node["label"] == "Youth in Nation-Building Act" for node in graph["nodes"]) == 1


def test_chunk_evidence_keeps_supporting_slide_only():
    evidence = text_extractor._chunk_evidence(
        {
            "id": 8,
            "slides": [4, 5],
            "text": "Slide 4\nRA 8044 created NYC.\nSlide 5\nBrexit changed debate.",
        },
        "RA 8044",
        "NYC",
    )

    assert [item["slides"] for item in evidence] == [[4]]
